...
\begin{align}
\mathcal{L} = &-\frac{1}{4} F^A_{\mu\nu}\; F_A^{\mu \nu} + \frac{1}{2} D^{ab}_\mu \phi_b \; D_{ab}^\mu \phi^b + i \psi_i^\dagger \sigma^\mu D^{ij}_\mu \psi_j  - \frac{1}{2}( m_{ij}\, \psi_i \,(i \sigma_2)\, \psi_j + \mathrm{h.c.} )\nonumber \\
&- \frac{1}{2}( Y^a_{ij} \phi_a \psi_i \,(i \sigma_2)\, \psi_j + \mathrm{h.c.} ) - m_{ab}\; \phi^a \phi^b - h_{abc}\; \phi^a \phi^b \phi^c - \lambda_{abcd}\; \phi^a \phi^b \phi^c \phi^d  \nonumber\\
&+ \mathcal{L}_{gf} + \mathcal{L}_{gh}
\end{align}
...
$\mathcal{L}_{gf}$ and $\mathcal{L}_{gh}$ represent the gauge fixing and ghost part respectively, however, most RGEs are gauge independent anyway with exception of anomalous dimensions, and hence vacuum expectation values. For those, $R_\xi$ gauge is implicitly assumed, yielding:
\begin{align}
